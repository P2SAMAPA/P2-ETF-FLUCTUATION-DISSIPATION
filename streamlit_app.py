...
st.set_page_config(page_title="Fluctuation-Dissipation Ratio", layout="wide")
...
st.markdown('<h1 style="text-align: center;">📊 Fluctuation-Dissipation Ratio</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Out-of-equilibrium markets | Effective temperature | Regime destabilization indicator</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 FDT")
...
st.sidebar.markdown(f"**Max lag:** {config.LAG_MAX} | **All macros used**")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'fdt_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>FDR: {etf['fdt_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized FDR"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized FDR", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("📊 Top ETFs by Fluctuation-Dissipation Ratio (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Fluctuation-Dissipation Theorem (FDT)** relates fluctuations to dissipation in equilibrium.
        - In out‑of‑equilibrium markets, the FDT is violated.
        - The **Fluctuation-Dissipation Ratio (FDR)** = (fluctuation) / (dissipation).
        - FDR = 1 in equilibrium; > 1 indicates out‑of‑equilibrium (larger fluctuations).
        - Higher FDR → farther from equilibrium → regime destabilization.
        - Macro variables modulate the FDR signal.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the FDR scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Fluctuation-Dissipation Ratio | Out-of-equilibrium market indicator")
